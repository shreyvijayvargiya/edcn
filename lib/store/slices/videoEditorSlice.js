...
const videoEditorSlice = createSlice({
	...
	reducers: {
		...
		openRecordScreenModal(state, action) {
			state.ui.recordScreenModal = action.payload ?? { insertAt: null };
		},
		closeRecordScreenModal(state) {
			state.ui.recordScreenModal = null;
		},
		...
		addRecordedVideo(state, action) {
			state.recordedVideos.unshift(action.payload);
		},
		removeRecordedVideo(state, action) {
			const id = action.payload;
			const track = state.recordedVideos.find((t) => t.id === id);
			if (track?.src?.startsWith("blob:")) {
				try {
					URL.revokeObjectURL(track.src);
				} catch {
			...
			state.recordedVideos = state.recordedVideos.filter((t) => t.id !== id);
		},
