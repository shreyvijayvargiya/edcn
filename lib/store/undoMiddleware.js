import { pushHistorySnapshot, bumpPreviewContentVersion } from "./slices/videoEditorSlice";
import {
...
export function undoMiddleware(store) {
	return (next) => (action) => {
		...
		if (
			HISTORY_TRACKED_ACTIONS.has(action.type) ||
			action.type === "videoEditor/undo" ||
			action.type === "videoEditor/redo"
		) {
			store.dispatch(bumpPreviewContentVersion());
		}
		return result;
	};
}
