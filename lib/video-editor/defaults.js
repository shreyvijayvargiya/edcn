...
import { DEFAULT_TIMELINE_PX_PER_SEC } from "./timeline";
...
import { DEFAULT_MEDIA_LAYER_STYLE, cloneMediaEffects } from "./mediaLayerStyle";
import { getUiPresetById, defaultUiPlacement } from "./uiComponents";
import { defaultCaptionData, defaultCaptionPlacement } from "./captions";
import { DEFAULT_DEMO_ANNOTATIONS } from "./demoAnnotations";
...
export function createVideoLayer(overrides = {}) {
	return {
		...
		data: {
			...
			muted: false,
			volume: 1,
			demoAnnotations: { ...DEFAULT_DEMO_ANNOTATIONS, markers: [] },
			...DEFAULT_MEDIA_LAYER_STYLE,
			effects: cloneMediaEffects(),
		},
		...overrides,
...
export function createCaptionLayer(overrides = {}) {
	const placement = defaultCaptionPlacement();
	return {
		id: uid("layer"),
		type: "caption",
		...timelineFields(),
		...
		visible: true,
		locked: false,
		animation: { ...DEFAULT_LAYER_ANIMATION, preset: "fade_in", duration: 0.35 },
		data: defaultCaptionData(),
		...overrides,
	};
}
...
export function createUiLayer(presetId = "cta-primary", overrides = {}) {
	const preset = getUiPresetById(presetId) ?? getUiPresetById("cta-primary");
	const placement = defaultUiPlacement(preset.size);
	return {
		id: uid("layer"),
		type: "ui",
		...timelineFields(),
		...
		data: {
			...
			...JSON.parse(JSON.stringify(preset.data)),
		},
		...overrides,
	};
}
...
export function createScene(overrides = {}) {
	...
	return {
		...
		name: "Scene 1",
		duration,
		timelinePxPerSec: DEFAULT_TIMELINE_PX_PER_SEC,
