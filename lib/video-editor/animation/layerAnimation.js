...
function computeEnterState(layer, progress, preset) {
...
function computeExitState(layer, progress, preset) {
	if (preset === "none" || progress <= 0) return null;
	const eased = easeOutCubic(progress);
	switch (preset) {
		case "fade_out":
			return { opacityMult: 1 - eased };
		case "slide_up":
			return { opacityMult: 1 - eased, offsetY: -eased * 56 };
		case "slide_down":
			return { opacityMult: 1 - eased, offsetY: eased * 56 };
		case "slide_left":
			return { opacityMult: 1 - eased, offsetX: -eased * 72 };
		case "slide_right":
			return { opacityMult: 1 - eased, offsetX: eased * 72 };
		case "zoom_out":
			return {
		...
		case "pop_out": {
			const s = 1 + 0.35 * easeOutBack(progress);
			return { opacityMult: 1 - eased, scaleX: s, scaleY: s };
		}
		case "spin_out":
			return {
				opacityMult: 1 - eased,
				rotationOffset: eased * 120,
				scaleX: 1 - 0.2 * eased,
				scaleY: 1 - 0.2 * eased,
			};
		default:
...
function mergeAnim(base, exit) {
	if (!exit) return base;
	return {
		...base,
		opacityMult: (base.opacityMult ?? 1) * (exit.opacityMult ?? 1),
		offsetX: (base.offsetX ?? 0) + (exit.offsetX ?? 0),
		offsetY: (base.offsetY ?? 0) + (exit.offsetY ?? 0),
		scaleX: (base.scaleX ?? 1) * (exit.scaleX ?? 1),
		scaleY: (base.scaleY ?? 1) * (exit.scaleY ?? 1),
		rotationOffset: (base.rotationOffset ?? 0) + (exit.rotationOffset ?? 0),
	};
}

/** Map clip-local time through loop / ping-pong for enter animation replay. */
function loopAdjustedLocalTime(layer, localTime) {
	const loop = layer.animation?.loop ?? "none";
	if (loop === "none") return localTime;

	const start = layer.startTime || 0;
	const clipDur = Math.max(0.25, layer.clipDuration ?? 5);
	const enterDur = Math.max(
		0.1,
		layer.animation?.duration ?? DEFAULT_LAYER_ANIMATION.duration,
	);
	const cycle = Math.max(enterDur, 0.5);
	const rel = Math.max(0, localTime - start);
	const count = layer.animation?.loopCount ?? 0;
	const maxCycles = count > 0 ? count : Math.ceil(clipDur / cycle);
	const cycleIndex = Math.min(Math.floor(rel / cycle), Math.max(0, maxCycles - 1));
	const inCycle = rel - cycleIndex * cycle;

	if (loop === "pingpong" && cycleIndex % 2 === 1) {
		return start + (cycle - inCycle);
	}
	return start + inCycle;
}

export function computeLayerAnimationState(layer, localTime) {
	const anim = layer.animation ?? DEFAULT_LAYER_ANIMATION;
	const preset = anim.preset ?? "none";
	const adjustedTime = loopAdjustedLocalTime(layer, localTime);
	const progress = getLayerAnimationProgress(
		{ ...layer, animation: anim },
		adjustedTime,
	);
	const enter = computeEnterState(layer, progress, preset);

	const exitPreset = anim.exitPreset ?? "none";
	const exitProgress =
		exitPreset === "none"
			? 0
			: exitAnimProgress(
					localTime,
					layer,
					anim.exitDuration ?? DEFAULT_LAYER_ANIMATION.exitDuration,
				);
	const exit = computeExitState(layer, exitProgress, exitPreset);

	return mergeAnim(enter, exit);
}
